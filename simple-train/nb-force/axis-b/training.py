...
def main():
    ...
    for t in xrange(trajNmb):
        ...
        S_PAxes = princAxes(PrincAxes_FILE,S_beadCor,S_beadName,S_beadMass)
...
def princAxes(PrincAxes_FILE,S_beadCor,S_beadName,S_beadMass):
    ''' Get the principal axis system for beads
     (using five beads ---- bead and the four neighboring beads)'''

    S_PAxes = []
    for chn in xrange(chnNmb):
        C_beadCor = S_beadCor[beadChnNmb*chn : beadChnNmb*(chn+1)]
        C_beadMass = S_beadMass[beadChnNmb*chn : beadChnNmb*(chn+1)]
        C_beadName = S_beadName[beadChnNmb*chn : beadChnNmb*(chn+1)]
        for i in xrange(beadChnNmb):
            if i == 0 or i == 1:
                beadCor = C_beadCor[0 : 5]
                beadMass = C_beadMass[0 : 5]
            elif i == beadChnNmb-2 or i == beadChnNmb-1: 
                beadCor = C_beadCor[beadChnNmb-5 : beadChnNmb]
                beadMass = C_beadMass[beadChnNmb-5 : beadChnNmb]
            else:
                beadCor = C_beadCor[i-2 : i+3]
                beadMass = C_beadMass[i-2 : i+3]
            coords = beadCor - C_beadCor[i]
            coords2 = sum(np.dot(beadMass,coords*coords)) * np.identity(3)
            inertia = coords2 - np.dot(beadMass * coords.transpose(),coords)
            eigvals,eigvecs = np.linalg.eig(inertia)
            # center of mass of five beads                                                                    
            com = sum([beadMass[j]*beadCor[j] for j in xrange(5)]) / sum(beadMass)
            # Orient first two eigenvectors so that com is in positive quadrant.                               
            for v in xrange(3):
                if np.dot(com - C_beadCor[i],eigvecs[v]) < 0.:
                    eigvecs[v] *= -1.   
            S_PAxes.append(eigvecs)
            PrincAxes_FILE.write(str(chn+1)+C_beadName[i]+'\t'+
                                 str(eigvals[0])+'\t'+str(eigvecs[0][0])+'\t'+str(eigvecs[0][1])+'\t'+str(eigvecs[0][2])+'\t'+
                                 str(eigvals[1])+'\t'+str(eigvecs[1][0])+'\t'+str(eigvecs[1][1])+'\t'+str(eigvecs[1][2])+'\t'+
                                 str(eigvals[2])+'\t'+str(eigvecs[2][0])+'\t'+str(eigvecs[2][1])+'\t'+str(eigvecs[2][2])+'\n')
#    print S_PAxes
#    exit()
    return S_PAxes
