...
def main():
    ...
    X_train, y_train, beadName_train, numBeadTyp_train, \
                          X_test, y_test, beadName_test = \
                        segregateDataSet(TS_CMatr,TS_Frc,S_beadName)   
...
def segregateDataSet(TS_CMatr,TS_Frc,S_beadName):
    '''Segregate data into training and test sets.'''
    ...
    FrcFlat = [TS_Frc[t][monNmb*2*chn+i] for t in xrange(trajNmb) for chn in xrange(chnNmb) for i in xrange(1,monNmb*2)]
    ...
    for i in xrange(dataSize):
        if i not in eleChosen:
            ...
            if chooseData:
                ...
                y_test.append(FrcFlat[i])
                beadName_test.append(beadNameFlat[i])
    numBeadTyp_test = {'A':0, 'B':0,}
    numBeadTyp_test['A'] = sum(1 for name in beadName_test if name[0] == 'A')
    numBeadTyp_test['B'] = sum(1 for name in beadName_test if name[0] == 'B')
    ...
    return X_trainFlat, y_trainFlat, beadName_trainFlat, numBeadTyp_train, \
                                 X_testFlat, y_testFlat, beadName_testFlat
